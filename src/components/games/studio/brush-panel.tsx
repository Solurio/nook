    ...
    <Panel title={tx("brushes")} onClose={onClose} className="top-2 left-12 w-80">
      ...
      {tab === "list" && (
        <div className="space-y-2">
          {MODES.map((mode) => {
            ...
            return (
              <div key={mode.id}>
                <p className="mb-1 text-[9px] tracking-wide text-muted/60 uppercase">{tx(mode.name)}</p>
                <div className="space-y-0.5">
                  {list.map((b) => (
                    ...
                    >
                      ...
                      <span className="min-w-0 flex-1 truncate text-[11px] text-chalk">{tx(b.name)}</span>
                      {custom.some((c) => c.id === b.id) && <span className="text-[9px] text-glow">{tx("made here")}</span>}
                    </button>
                  ))}
                </div>
              </div>
      ...
      {tab === "edit" && (
        <div className="space-y-1.5">
          ...
          <Range label={tx("size")} value={Math.round(spec.size)} min={1} max={500} onChange={(v) => set({ size: v })} format={(v) => `${v}px`} />
