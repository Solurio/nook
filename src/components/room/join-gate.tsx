      ...
      <div className="animate-drift-in surface relative w-full max-w-sm rounded-3xl p-6">
        <div className="mb-5 flex items-center gap-2.5">
          ...
          <div className="min-w-0">
            <p className="truncate text-sm font-semibold">{room?.name ?? t("a nook")}</p>
            <p className="text-xs text-muted">
              {alreadyHere > 0
                ? t(`${alreadyHere} ${alreadyHere === 1 ? "person is" : "people are"} inside`)
                : t("nobody here yet")}
            </p>
          </div>
        </div>

        <label className="mb-1.5 block text-xs font-medium text-muted">{t("what should we call you?")}</label>
        ...
        <p className="mt-4 mb-2 text-xs font-medium text-muted">{t("your colour")}</p>
